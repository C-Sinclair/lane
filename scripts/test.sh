...
# surviving command set (init, new, ls, enter/switch, exit, rm, prune, shellenv).
# The clone layer and anchor-free worktree logic are covered in depth by `cargo test`.
set -uo pipefail
...
is "ls prints the lane name once" \
   "$("$LANE" ls | awk '$1 == "fix-login" && $2 == "open" && $3 == "clean" { n++ } END { print n + 0 }')" "1"
is "ls json reports the exact clean open lane row" \
   "$("$LANE" ls --json | python3 -c 'import json,sys; d=json.load(sys.stdin)[0]; print(int(d["name"]=="fix-login" and d["path"]==sys.argv[1] and d["branch"]=="fix-login" and d["state"]=="open" and d["dirty"] is False))' "$LP_REAL")" "1"
...
echo "== 7. enter/switch and exit move the shell =="
setup
"$LANE" new here > /dev/null 2>&1
LP_REAL=$(cd .lane/trees/here && pwd -P)
is "enter prints the lane path" "$("$LANE" enter here)" "$LP_REAL"
is "switch is an alias for enter" "$("$LANE" switch here)" "$LP_REAL"
ROOT_REAL=$(pwd -P)
is "exit prints the repo root" "$("$LANE" exit)" "$ROOT_REAL"
is "enter without shell integration warns on a terminal only" \
   "$("$LANE" enter here 2>&1 >/dev/null | grep -c 'shell integration')" "0"
...
"$LANE" new pushed-away > /dev/null 2>&1
...
setup
"$LANE" new open-work > /dev/null 2>&1
...
"$LANE" new dry-landed > /dev/null 2>&1
...
setup
"$LANE" new stateful > /dev/null 2>&1
...
"$LANE" rm stateful --force > /dev/null 2>&1
...
"$LANE" new gc-proof > /dev/null 2>&1
